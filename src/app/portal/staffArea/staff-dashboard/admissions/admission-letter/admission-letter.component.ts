...
export class AdmissionLetterComponent implements OnInit {
    ...
    public showInput() {
        this.showInputText = this.testText;
    }

    public logUserInput($event) {
        // let textContent = $event.event.target.parentElement.lastChild.innerHTML;
        this.testText = $event.innerHTML;
        console.log("Log user input event ", this.testText);
    }

    public triggerImageInfo(modalId) {
        console.log("Image insertedddddd oo");
        this.triggerModalOrOverlay('open', modalId);
    }
