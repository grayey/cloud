...
export class JambRecordComponent implements OnInit {
    ...
    private getAllJambRecords() {
        this.load.requesting.list = true;
        this.admissionsService.getAllJambRecords().subscribe(
            (allJambRecordsResponse) => {
                this.load.requesting.list = false;
                this.allJambRecords = allJambRecordsResponse.data;
    ...
    private async createJambRecord() {
    ...
    public triggerModalOrOverlay(action: string, modalId: string, ind?: number) {
        if (ind >= 0) {
            this.allJambRecords[ind].loading = true;
            this.getAllCourses();
            this.updatedJambRecord = this.allJambRecords[ind];
            const JambRecordManagementObject = {
                first_name: this.updatedJambRecord.first_name,
                last_name: this.updatedJambRecord.last_name,
                middle_name: "",
                matric_no: this.updatedJambRecord.matric_no,
                email: this.updatedJambRecord.email,
                course_of_study_id: this.updatedJambRecord.course_of_study.id,
                department_id: this.updatedJambRecord.course_of_study.department.id,
                faculty_id: this.updatedJambRecord.course_of_study.department.faculty.id,
                country_id: 1,
                state_id: 2
            }
            this.updateJambRecordForm.patchValue(JambRecordManagementObject);
            console.log("JambRecord management object ", JambRecordManagementObject);
